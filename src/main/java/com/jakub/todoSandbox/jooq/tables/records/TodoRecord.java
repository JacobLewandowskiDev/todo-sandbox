/*
 * This file is generated by jOOQ.
 */
package com.jakub.todoSandbox.jooq.tables.records;


import com.jakub.todoSandbox.jooq.enums.PriorityEnum;
import com.jakub.todoSandbox.jooq.tables.Todo;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record4;
import org.jooq.Row4;
import org.jooq.impl.UpdatableRecordImpl;


/**
 * This class is generated by jOOQ.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class TodoRecord extends UpdatableRecordImpl<TodoRecord> implements Record4<Long, String, String, PriorityEnum> {

    private static final long serialVersionUID = 1L;

    /**
     * Setter for <code>public.todo.id</code>.
     */
    public void setId(Long value) {
        set(0, value);
    }

    /**
     * Getter for <code>public.todo.id</code>.
     */
    public Long getId() {
        return (Long) get(0);
    }

    /**
     * Setter for <code>public.todo.name</code>.
     */
    public void setName(String value) {
        set(1, value);
    }

    /**
     * Getter for <code>public.todo.name</code>.
     */
    public String getName() {
        return (String) get(1);
    }

    /**
     * Setter for <code>public.todo.description</code>.
     */
    public void setDescription(String value) {
        set(2, value);
    }

    /**
     * Getter for <code>public.todo.description</code>.
     */
    public String getDescription() {
        return (String) get(2);
    }

    /**
     * Setter for <code>public.todo.priority</code>.
     */
    public void setPriority(PriorityEnum value) {
        set(3, value);
    }

    /**
     * Getter for <code>public.todo.priority</code>.
     */
    public PriorityEnum getPriority() {
        return (PriorityEnum) get(3);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    @Override
    public Record1<Long> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record4 type implementation
    // -------------------------------------------------------------------------

    @Override
    public Row4<Long, String, String, PriorityEnum> fieldsRow() {
        return (Row4) super.fieldsRow();
    }

    @Override
    public Row4<Long, String, String, PriorityEnum> valuesRow() {
        return (Row4) super.valuesRow();
    }

    @Override
    public Field<Long> field1() {
        return Todo.TODO.ID;
    }

    @Override
    public Field<String> field2() {
        return Todo.TODO.NAME;
    }

    @Override
    public Field<String> field3() {
        return Todo.TODO.DESCRIPTION;
    }

    @Override
    public Field<PriorityEnum> field4() {
        return Todo.TODO.PRIORITY;
    }

    @Override
    public Long component1() {
        return getId();
    }

    @Override
    public String component2() {
        return getName();
    }

    @Override
    public String component3() {
        return getDescription();
    }

    @Override
    public PriorityEnum component4() {
        return getPriority();
    }

    @Override
    public Long value1() {
        return getId();
    }

    @Override
    public String value2() {
        return getName();
    }

    @Override
    public String value3() {
        return getDescription();
    }

    @Override
    public PriorityEnum value4() {
        return getPriority();
    }

    @Override
    public TodoRecord value1(Long value) {
        setId(value);
        return this;
    }

    @Override
    public TodoRecord value2(String value) {
        setName(value);
        return this;
    }

    @Override
    public TodoRecord value3(String value) {
        setDescription(value);
        return this;
    }

    @Override
    public TodoRecord value4(PriorityEnum value) {
        setPriority(value);
        return this;
    }

    @Override
    public TodoRecord values(Long value1, String value2, String value3, PriorityEnum value4) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached TodoRecord
     */
    public TodoRecord() {
        super(Todo.TODO);
    }

    /**
     * Create a detached, initialised TodoRecord
     */
    public TodoRecord(Long id, String name, String description, PriorityEnum priority) {
        super(Todo.TODO);

        setId(id);
        setName(name);
        setDescription(description);
        setPriority(priority);
    }
}
